...
namespace agsXMPP.ui.roster
{
    public class GroupJid
    {
        ...
        public GroupJid(string group, Jid jid)
        {
            this.Group = group;
            this.Jid = jid;
        }

        public GroupJid(Jid jid)
        { 
        ...
        {
            return this.Group +":"+this.Jid.ToString();
        }
    }
    ...
    public partial class RosterControl : UserControl, IRosterControl
    {
        ...
        public event System.Windows.Forms.MouseEventHandler TreeViewMouseDown;              

        #region << Properties >>
        private Dictionary<GroupJid, RosterData> m_Roster = new Dictionary<GroupJid, RosterData>();
        ...
        /// </summary>
        public Dictionary<GroupJid, RosterData> Roster
        {
            get { return m_Roster; }
        }
        ...
        public void ExpandOnline()
        {
            m_RootOnline.ExpandAll();
        }

        public RosterNode GetRosterItem(string group, Jid jid)
        {
            GroupJid gid = new GroupJid(group, jid);

            if (m_Roster.ContainsKey(gid))
            {
                RosterData d = m_Roster[gid];
                return d.RosterNode;
            }
            else
            {
        ...
        {
            foreach (KeyValuePair<GroupJid, RosterData> kvp in m_Roster)
            {
                GroupJid gid = kvp.Key;
                if (gid.Jid.Bare == jid.Bare)
                {
                    RosterData d = m_Roster[gid];
                    return d.RosterNode;
                }
        ...
        public bool RemoveRosterItem(agsXMPP.protocol.iq.roster.RosterItem ritem)
        {
            ...
            {
                for (int i = 0; i < ritem.GetGroups().Count; i++)
                {
                    Group g = (Group)ritem.GetGroups().Item(i);
                    GroupJid gid = new GroupJid(g.Name, ritem.Jid);
                    RemoveRosterItem(gid);
                }
            }

            return RemoveRosterItem(new GroupJid(ritem.Jid));
        }
        ...
        /// and could not be removed
        /// </returns>
        public bool RemoveRosterItem(GroupJid gid)
        {
        ...
        public RosterNode AddRosterItem(agsXMPP.protocol.iq.roster.RosterItem ritem)
        {
            ...
            if (ritem.GetGroups().Count > 0)
            {
                RosterNode rosterNode = null;
                for (int k = 0; k < ritem.GetGroups().Count; k++)
                {
                    string groupname;
                    Group g = (Group)ritem.GetGroups().Item(k);
                    groupname = g.Name;

                    GroupJid gid = new GroupJid(groupname, ritem.Jid);
                    rosterNode = AddRosterItem(ritem, nodeText, gid);
                }
            ...
            GroupJid gid1 = new GroupJid(ritem.Jid);
            return AddRosterItem(ritem, nodeText, gid1);
        

        }

        private RosterNode AddRosterItem(agsXMPP.protocol.iq.roster.RosterItem ritem, string nodeText, GroupJid gid)
        {
            RosterNode node;
            string groupname = gid.Group;
            ...
            if (!m_Roster.ContainsKey(gid))
            {
                ...
                m_Roster.Add(gid, new RosterData(node));

                return node;
            }


            if (m_Roster[gid].Online)
            {
                ...
                // new Node again                  
                Presence[] pres = new Presence[m_Roster[gid].Presences.Count];
                int i = 0;
                Dictionary<string, PresenceData>.Enumerator myEnum = m_Roster[gid].Presences.GetEnumerator();
        ...
        public void SetPresence(Presence pres)
        {
            ...
            {
                GroupJid gid = kvp.Key;
                if (pres.From.Bare != gid.Jid.Bare)
                    continue;
        ...
        private void treeView_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (node != null)
            {
                ...
                if (TreeViewMouseDown != null)
                    TreeViewMouseDown(this, e);
            }
        }
            ...
            if (rn != null && rn.NodeType == RosterNodeType.RosterNode)
            {
                ...
                if (currentNodeIndex != oldNodeIndex)
                {
                    ...
                    if (rn.RosterItem != null)
                    {
                        ...
                        {
                            GroupJid gid = kvp.Key;
                            if (rn.RosterItem.Jid.Bare != gid.Jid.Bare)
                                continue;
