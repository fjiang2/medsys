...
namespace Sys.Data.Manager
{
	...
	public class MigrationProvider 
	{
        ...
        public void AddPrimaryKey(string name, TableName table, params string[] columns)
        {
            ...
            DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} ADD CONSTRAINT {1} PRIMARY KEY ({2}) ", table, name, string.Join(",", columns));
        }
        ...
        public  void RemoveConstraint(TableName table, string name)
		{
			...
			{
                DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} DROP CONSTRAINT {1}", table, name);
			}
		}
		...
		{
			if (TableExists(name))
                DataExtension.ExecuteNonQuery(name.Provider, "DROP TABLE {0}", name);
		}
        ...
        public void RenameTable(TableName oldName, TableName newName)
		{
			...
			if (TableExists(oldName))
                DataExtension.ExecuteNonQuery(newName.Provider, "ALTER TABLE {0} RENAME TO {1}", oldName, newName);
		}
        ...
        {
            return table.GetTableSchema().Columns.Exists(column);
        }
        ...
        public void AddColumn(TableName table, string column, SqlDbType type, int size, object defaultValue)
        {
            ...
            DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} ADD {1} {2} NULL", table, column, type);
        }
        ...
        public void RenameColumn(TableName tableName, string oldColumnName, string newColumnName)
		{
			...
			if (ColumnExists(tableName, oldColumnName))
                DataExtension.ExecuteNonQuery(tableName.Provider, "ALTER TABLE {0} RENAME COLUMN {1} TO {2}", tableName, oldColumnName, newColumnName);
		}
        ...
        public void RemoveColumn(TableName table, string column)
		{
			if (ColumnExists(table, column))
			{
                ...
                DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} DROP COLUMN {1} ", table, column);
			}
		}
        ...
        public void ChangeColumn(TableName table, string sqlColumn)
		{
            ...
            DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} ALTER COLUMN {1}", table, sqlColumn);
		}
        ...
        public void AddUniqueConstraint(string name, TableName table, params string[] columns)
		{
            ...
            DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} ADD CONSTRAINT {1} UNIQUE({2}) ", table, name, string.Join(", ", columns));
		}
        ...
        public void AddCheckConstraint(string name, TableName table, string checkSql)
		{
            ...
            DataExtension.ExecuteNonQuery(table.Provider, "ALTER TABLE {0} ADD CONSTRAINT {1} CHECK ({2}) ", table, name, checkSql);
		}
        ...
        {
            return DataExtension.FillDataRow(table.Provider, "SELECT TOP 1 * FROM sysobjects WHERE id = object_id('{0}')", name) != null;
        }
        ...
        private void DeleteColumnConstraints(TableName table, string column)
        {
            ...
            DataTable dt = DataExtension.FillDataTable(table.Provider, SQL);
