...
namespace Sys.Data
{
    public class ComputedColumns
    {
        ...
        internal ComputedColumns(MetaColumnCollection columns)
        {
            this.keys = columns.Where(column => column.IsComputed).Select(column => column.ColumnName).ToArray();
        }

        public ComputedColumns(TableName tname)
        { 
            string SQL = @"
            USE [{0}]
            SELECT c.name
